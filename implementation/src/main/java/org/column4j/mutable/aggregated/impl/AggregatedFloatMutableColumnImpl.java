...
import org.column4j.mutable.aggregated.AggregatedFloatMutableColumn;
 ...
 * @since 0.0.1
 */
public class AggregatedFloatMutableColumnImpl implements AggregatedFloatMutableColumn {

    private final int chunkSize;
    private final float tombstone;
    private final FloatMutableColumn mutableColumn;
    private final Map<Integer, FloatStatistic> chunkedColumnStatistics;

    public AggregatedFloatMutableColumnImpl(FloatMutableColumn mutableColumn, int chunkSize) {
    ...
    private void initStatistic(int chunks, int dataSize) {
        ...
        for (var chunk = 0; chunk < chunks; chunk++) {
            ...
            FloatStatistic chunkStatistic;
            if (from >= dataSize) {
                chunkStatistic = new FloatStatistic(tombstone);
            } else {
                chunkStatistic = FloatStatisticUtils.collectStatistic(data, dataSize, tombstone, from, to);
            }
    ...
    private void onValueSet(int position, float oldValue, float newValue) {
        var intStat = getChunkStatistic(position);
        var data = getData();
        intStat.onValueSet(data, position, oldValue, newValue);
    }

    private FloatStatistic getChunkStatistic(int position) {
        var chunk = position / chunkSize;
        return chunkedColumnStatistics.computeIfAbsent(
                chunk,
                it -> new FloatStatistic(tombstone)
        );
    }

    private float getValueOrTombstone(int position) {
        var oldData = getData();
    ...
    @Override
    public Map<Integer, FloatStatistic> getChunkedColumnStatistics() {
