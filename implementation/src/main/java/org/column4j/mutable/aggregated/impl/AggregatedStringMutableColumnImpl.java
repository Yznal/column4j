...
import org.column4j.mutable.aggregated.AggregatedStringMutableColumn;
 ...
 * @since 0.0.1
 */
public class AggregatedStringMutableColumnImpl implements AggregatedStringMutableColumn {

    private final int chunkSize;
    private final String tombstone;
    private final StringMutableColumn mutableColumn;
    private final Map<Integer, StringStatistic> chunkedColumnStatistics;

    public AggregatedStringMutableColumnImpl(StringMutableColumn mutableColumn, int chunkSize) {
    ...
    private void initStatistic(int chunks, int dataSize) {
        ...
        for (var chunk = 0; chunk < chunks; chunk++) {
            ...
            StringStatistic chunkStatistic;
            if (from >= dataSize) {
                chunkStatistic = new StringStatistic(tombstone);
            } else {
                chunkStatistic = StringStatisticUtils.collectStatistic(data, dataSize, tombstone, from, to);
            }
    ...
    private void onValueSet(int position, String oldValue, String newValue) {
        var intStat = getChunkStatistic(position);
        var data = getData();
        intStat.onValueSet(data, position, oldValue, newValue);
    }

    private StringStatistic getChunkStatistic(int position) {
        var chunk = position / chunkSize;
        return chunkedColumnStatistics.computeIfAbsent(
                chunk,
                it -> new StringStatistic(tombstone)
        );
    }

    private String getValueOrTombstone(int position) {
        var oldData = getData();
    ...
    @Override
    public Map<Integer, StringStatistic> getChunkedColumnStatistics() {
